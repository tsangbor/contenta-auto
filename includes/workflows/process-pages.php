 ...
 * 共用的頁面生成工作流程
 * 
 * 此檔案包含頁面生成的完整邏輯，被 step-14.php 和 step-14-luke.php 共用
 * 保持與原本 step-14.php 完全相同的功能和邏輯
 * 
 * 使用全域變數進行參數傳遞：
 ...
 * - $config: 配置管理器
 * - $job_id: Job ID
 * - $document_root: WordPress 安裝路徑
 * 
 * @package Contenta
 * @version 2.1 (基於原本 step-14.php 完整功能)
 */

// 確保必要的變數已定義
if (!isset($wp_cli) || !isset($deployer) || !isset($work_dir) || !isset($config)) {
    throw new Exception('必要的全域變數未定義：$wp_cli, $deployer, $work_dir, $config');
}

if (!isset($document_root) || !isset($job_id)) {
    throw new Exception('必要的變數未定義：$document_root, $job_id');
}
...
class PageGenerator {
    ...
    public function __construct($deployer, $job_id, $config, $wp_cli, $document_root) {
    ...
    public function generatePages() {
        ...
        // 取得頁面清單 - 適應新舊資料結構
        $work_dir = DEPLOY_BASE_PATH . '/data/' . $this->job_id;
        $data_file = $work_dir . '/' . $this->job_id . '.json';
        ...
        if (!file_exists($data_file)) {
            ...
            if (!file_exists($site_config_file)) {
                $this->deployer->log("錯誤: 找不到頁面配置檔案");
                return false;
            }
            
            $site_config = json_decode(file_get_contents($site_config_file), true);
            $page_list_array = $site_config['page_list'] ?? [];
        } else {
            // 新的資料結構
            $job_data = json_decode(file_get_contents($data_file), true);
            $page_list_array = $job_data['confirmed_data']['page_list'] ?? $job_data['page_list'] ?? [];
        }
        
        if (empty($page_list_array)) {
            $this->deployer->log("警告: 沒有定義頁面清單");
            return false;
        }
        
        // 轉換為關聯陣列格式 (slug => title)
        $page_list = [];
        if (is_array($page_list_array)) {
            foreach ($page_list_array as $key => $value) {
                if (is_numeric($key)) {
                    // 索引陣列格式 ['home', 'about', 'service']
                    $slug = $value;
                    $title = ucfirst($slug);
                } else {
                    // 關聯陣列格式 ['home' => '首頁', 'about' => '關於我們']
                    $slug = $key;
                    $title = $value;
                }
                ...
                // 設定中文標題
                switch ($slug) {
                    case 'home': $title = '首頁'; break;
                    case 'about': $title = '幕後故事'; break;
                    case 'blog': $title = '部落格'; break;
                    case 'service': $title = '服務'; break;
                    case 'contact': $title = '聯絡我們'; break;
                }
    ...
    private function createPageViaWPCLI($page_title, $page_slug, $page_json) {
        ...
        if (!file_exists($data_file)) {
            // 回退到舊的路徑結構
            $work_dir = DEPLOY_BASE_PATH . '/temp/' . $this->job_id;
            $data_file = $work_dir . '/config/processed_data.json';
        }
        
        $job_data = json_decode(file_get_contents($data_file), true);
        $domain = $job_data['confirmed_data']['domain'] ?? $job_data['domain'] ?? null;
        $user_email = $job_data['confirmed_data']['user_email'] ?? $job_data['user_email'] ?? 'admin@' . $domain;
        
        // 準備遠端檔案路徑
        $remote_temp_file = "{$this->document_root}/wp-content/uploads/temp/{$page_slug}-ai.json";
        ...
        // 確保遠端 temp 目錄存在
        $create_dir_cmd = "mkdir -p {$this->document_root}/wp-content/uploads/temp";
        ...
        $create_page_cmd = "cd {$this->document_root} && wp post create --post_type=page --post_title=" . escapeshellarg($page_title) . " --post_name=" . escapeshellarg($page_slug) . " --post_status=publish --user=" . escapeshellarg($user_email) . " --porcelain --allow-root";
        ...
        $import_cmd = "cd {$this->document_root} && cat {$remote_temp_file} | wp post meta update {$page_id} _elementor_data --format=json --allow-root";
        ...
        foreach ($meta_updates as $meta_key => $meta_value) {
            $update_meta_cmd = "cd {$this->document_root} && wp post meta update {$page_id} {$meta_key} " . escapeshellarg($meta_value) . " --allow-root";
        ...
        // 第四步：驗證匯入結果
        $verify_cmd = "cd {$this->document_root} && wp post get {$page_id} --field=post_title --allow-root";
    ...
    private function setHomePage() {
        ...
        $find_home_cmd = "cd {$this->document_root} && wp post list --post_type=page --name=home --format=csv --fields=ID --allow-root";
        ...
        $set_show_on_front_cmd = "cd {$this->document_root} && wp option update show_on_front page --allow-root";
        ...
        $set_page_on_front_cmd = "cd {$this->document_root} && wp option update page_on_front {$home_page_id} --allow-root";
        ...
        $find_blog_cmd = "cd {$this->document_root} && wp post list --post_type=page --name=blog --format=csv --fields=ID --allow-root";
        ...
        if ($blog_result['return_code'] === 0) {
            ...
            if (count($blog_lines) >= 2) {
                ...
                if (is_numeric($blog_page_id)) {
                    ...
                    $set_page_for_posts_cmd = "cd {$this->document_root} && wp option update page_for_posts {$blog_page_id} --allow-root";
        ...
        // 第四步：驗證設定結果
        $verify_cmd = "cd {$this->document_root} && wp option get show_on_front --allow-root && echo '---' && wp option get page_on_front --allow-root";
        $verify_result = executeSSH($server_host, $ssh_user, $ssh_port, $ssh_key_path, $verify_cmd);
...
// ========== 主要處理流程 ==========
$deployer->log("=== 開始頁面生成工作流程 ===");

$generator = new PageGenerator($deployer, $job_id, $config, $wp_cli, $document_root);
$success = $generator->generatePages();

if ($success) {
    $deployer->log("=== 頁面生成工作流程完成 ===");
    return ['status' => 'success', 'message' => '頁面生成完成'];
} else {
    $deployer->log("=== 頁面生成工作流程失敗 ===");
    return ['status' => 'error', 'message' => '頁面生成失敗'];
}