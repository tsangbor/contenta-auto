...
// 🚨 最重要：首先檢查 article-prompts.json
$article_prompts_file = $work_dir . '/json/article-prompts.json';
if (!file_exists($article_prompts_file)) {
    $deployer->log("⚠️  警告: article-prompts.json 檔案不存在: {$article_prompts_file}");
    $deployer->log("🛑 停止所有接下來步驟：找不到必要的文章提示詞檔案");
    return ['status' => 'error', 'message' => '⚠️ article-prompts.json 不存在，停止處理'];
}

$prompts = json_decode(file_get_contents($article_prompts_file), true);
if (!is_array($prompts) || empty($prompts)) {
    $deployer->log("⚠️  警告: article-prompts.json 格式無效或為空");
    $deployer->log("🛑 停止所有接下來步驟：article-prompts.json 格式錯誤");
    return ['status' => 'error', 'message' => '⚠️ article-prompts.json 格式無效，停止處理'];
}

// article-prompts.json 基本格式檢查通過

$deployer->log("✅ article-prompts.json 檢查通過，找到 " . count($prompts) . " 個文章策略");
...
$deployer->log("使用 OpenAI 服務: {$content_model}");
...
// 成本優化建議（僅供參考）
$deployer->log("成本優化分析:");
$recommended_model = Contenta_Cost_Optimizer::recommend_model('content_generation', 'standard');
$deployer->log("建議內容生成模型: {$recommended_model} (當前使用: {$content_model})");
 ...
 * 呼叫 Gemini API 生成內容
 */
function call_gemini_api($prompt, $model = 'gemini-2.5-flash', $max_tokens = 2000, $api_key = null) {
    global $deployer;
    ...
    if (empty($api_key)) {
        return ['success' => false, 'error' => 'Google API 金鑰未設定'];
    }
    ...
    $deployer->log("Gemini API 錯誤 - HTTP {$http_code}: {$response}");
    return ['success' => false, 'error' => $response];
}
...
// 載入必要資料
$base_path = defined('DEPLOY_BASE_PATH') ? DEPLOY_BASE_PATH : dirname(__DIR__, 2);
...
**同時請在文章結尾後，另起一行提供一個 100 字以內的英文摘要，格式如下：**
[SUMMARY]: 這裡放英文摘要內容，用於圖片生成";

        // 使用 OpenAI API
        $content_result = call_openai_api($content_prompt, $content_model, 3000, $openai_api_key);
        
        if (!$content_result['success']) {
            throw new Exception("文章內容生成失敗: " . ($content_result['error'] ?? '未知錯誤'));
        }
        
        $raw_content = clean_ai_content($content_result['content']);
        
        // 分離文章內容和摘要
        $article_content = $raw_content;
        $article_summary = '';
        
        if (preg_match('/\[SUMMARY\]:\s*(.+?)$/s', $raw_content, $matches)) {
            $article_summary = trim($matches[1]);
            $article_content = preg_replace('/\[SUMMARY\]:\s*.+$/s', '', $raw_content);
        ...
        if (!empty($article_summary)) {
            $deployer->log("  文章摘要: " . substr($article_summary, 0, 100) . "...");
        }
...
使用專業攝影風格，真實場景和人物，高品質視覺效果，色彩搭配: {$color_description}。";
        ...
        // 使用文章摘要而不是完整內容生成圖片提示詞
        $content_for_image = !empty($article_summary) ? $article_summary : substr(strip_tags($article_content), 0, 300);
        
        $image_prompt_instruction = "你是一位專業的藝術總監。請根據以下文章摘要，為其設計一張精選圖片。

文章標題：{$article_prompt['title']}
文章摘要：{$content_for_image}
...
請直接輸出英文提示詞，不要包含任何中文說明。";
        ...
        // 使用 OpenAI API
        $image_prompt_result = call_openai_api($image_prompt_instruction, $content_model, 300, $openai_api_key);
        
        if (!$image_prompt_result['success']) {
            throw new Exception("圖片提示詞生成失敗: " . ($image_prompt_result['error'] ?? '未知錯誤'));
        }
...
file_put_contents($logs_dir . '/article-generation-results.json', json_encode($generation_results, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE));

$deployer->log("詳細結果已儲存至: logs/article-generation-results.json");
