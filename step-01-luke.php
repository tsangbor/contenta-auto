<?php
/**
 * 步驟 01: Cloudflare DNS 設定 (Luke API 模式)
 * 透過 Cloudflare API 建立新網站，設定 DNS 記錄指向 Luke 主機
 * 
 * 關鍵修改：建立 A 記錄時，指向的 IP 位址必須是從設定檔讀取的 config['luke_deployment']['ssh_host']
 */
...
$deployer->log("開始 Cloudflare DNS 設定 (Luke 模式): {$domain}");

// 取得 API 憑證
$cf_email = $config->get('api_credentials.cloudflare.email');
$cf_api_key = $config->get('api_credentials.cloudflare.api_token');

// Luke 部署模式：從 luke_deployment 讀取主機 IP
$luke_config = $config->get('luke_deployment');
$server_ip = $luke_config['ssh_host'];

$deployer->log("Luke 主機 IP: {$server_ip}");
...
if (empty($server_ip)) {
    throw new Exception("未設定 Luke 主機 IP 位址 (luke_deployment.ssh_host)");
}
...
try {
    ...
    $deployer->log("Cloudflare 設定完成 (Luke 模式)");
    ...
    $deployer->log("Cloudflare 設定失敗 (Luke 模式): " . $e->getMessage());
