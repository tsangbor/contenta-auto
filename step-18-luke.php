<?php
/**
 * 步驟 18: Elementor Kit 全域設定匯入 (Luke API 模式)
 * 
 ...
 * 執行 SSH 指令（使用密碼認證）
 */
function executeSSHWithPassword($host, $username, $port, $password, $command)
{
    $ssh_command = sprintf(
        'sshpass -p %s ssh -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -p %d %s@%s %s 2>&1',
...
$deployer->log("=== 步驟 18: Elementor Kit 全域設定匯入 (Luke API 模式) ===");
...
try {
    ...
    // 取得 Luke 部署 SSH 連接參數
    $ssh_user = $deploy_config['luke_deployment']['ssh_user'] ?? '';
    $server_host = $deploy_config['luke_deployment']['ssh_host'] ?? '';
    $ssh_port = $deploy_config['luke_deployment']['ssh_port'] ?? 22;
    $ssh_password = $deploy_config['luke_deployment']['ssh_password'] ?? '';
    
    if (empty($server_host) || empty($ssh_user) || empty($ssh_password)) {
        throw new Exception("Luke 部署 SSH 配置不完整");
    }
    ...
    $scp_cmd = sprintf(
        'sshpass -p %s scp -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -P %d %s %s@%s:%s 2>&1',
        escapeshellarg($ssh_password),
        $ssh_port,
        escapeshellarg($kit_zip_path),
    ...
    $cleanup_result = executeSSHWithPassword($server_host, $ssh_user, $ssh_port, $ssh_password, $cleanup_cmd);
    ...
    $deployer->log("=== 步驟 18 完成：Elementor Kit 全域設定匯入 (Luke API 模式) ===");
    
    return [
        'status' => 'success',
        'message' => 'Elementor Kit 全域設定匯入成功 (Luke API 模式)',
        'details' => [
    ...
    $error_message = "步驟 18 錯誤 (Luke API 模式): " . $e->getMessage();
