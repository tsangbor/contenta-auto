<?php
/**
 * 步驟 14: 生成頁面 (Luke API 版本)
 * 根據組合的頁面-ai.json檔案，匯入到主機的頁面
 * 使用 Luke API 的 SSH 密碼認證和路徑結構
 */
...
/**
 * 執行 SSH 指令（含重試機制）- Luke API 版本
 * 支援 SSH 密碼認證和金鑰認證
 */
function executeSSH($host, $user, $port, $key_path, $command, $max_retries = 3, $password = null)
{
    // Luke 模式優先使用密碼認證
    if (!empty($password)) {
...
class PageGenerator {
    ...
    private function createPageViaWPCLI($page_title, $page_slug, $page_json) {
        // 取得 Luke API 部署設定
        $luke_config = $this->config->get('luke_deployment');
        if (!$luke_config) {
        ...
        $ssh_key_path = $luke_config['ssh_key_path'] ?? null;
        $ssh_password = $luke_config['ssh_password'] ?? null;
        ...
        executeSSH($server_host, $ssh_user, $ssh_port, $ssh_key_path, $create_dir_cmd, 3, $ssh_password);
        ...
        $create_result = executeSSH($server_host, $ssh_user, $ssh_port, $ssh_key_path, $create_page_cmd, 3, $ssh_password);
        ...
        $import_result = executeSSH($server_host, $ssh_user, $ssh_port, $ssh_key_path, $import_cmd, 3, $ssh_password);
        ...
        foreach ($meta_updates as $meta_key => $meta_value) {
            ...
            $meta_result = executeSSH($server_host, $ssh_user, $ssh_port, $ssh_key_path, $update_meta_cmd, 3, $ssh_password);
        ...
        $verify_result = executeSSH($server_host, $ssh_user, $ssh_port, $ssh_key_path, $verify_cmd, 3, $ssh_password);
        ...
        executeSSH($server_host, $ssh_user, $ssh_port, $ssh_key_path, $cleanup_cmd, 3, $ssh_password);
    ...
    private function setHomePage() {
        ...
        $find_result = executeSSH($server_host, $ssh_user, $ssh_port, $ssh_key_path, $find_home_cmd, 3, $ssh_password);
        ...
        $show_on_front_result = executeSSH($server_host, $ssh_user, $ssh_port, $ssh_key_path, $set_show_on_front_cmd, 3, $ssh_password);
        ...
        $page_on_front_result = executeSSH($server_host, $ssh_user, $ssh_port, $ssh_key_path, $set_page_on_front_cmd, 3, $ssh_password);
        ...
        $blog_result = executeSSH($server_host, $ssh_user, $ssh_port, $ssh_key_path, $find_blog_cmd, 3, $ssh_password);
        ...
        if ($blog_result['return_code'] === 0) {
            ...
            if (count($blog_lines) >= 2) {
                ...
                if (is_numeric($blog_page_id)) {
                    ...
                    $page_for_posts_result = executeSSH($server_host, $ssh_user, $ssh_port, $ssh_key_path, $set_page_for_posts_cmd, 3, $ssh_password);
