...
class JimengClient {
    ...
    getSignatureKey(key, dateStamp, regionName, serviceName) {
        const kDate = crypto_1.default.createHmac('sha256', key).update(dateStamp).digest();
        ...
        const kSigning = crypto_1.default.createHmac('sha256', kService).update('request').digest();
        return kSigning;
    }
    /**
     * 格式化查询参数
    ...
    signV4Request(reqQuery, reqBody) {
        ...
        const payloadHash = crypto_1.default.createHash('sha256').update(reqBody).digest('hex');
        ...
        const signature = crypto_1.default.createHmac('sha256', signingKey).update(stringToSign).digest('hex');
        if (this.debug) {
    ...
    async generateImage(params) {
        ...
        while (retryCount <= this.retries) {
            try {
                ...
                // 发送请求
                const response = await axios_1.default.post(requestUrl, bodyParams, {
                    headers: headers,
     ...
     * 查询视频生成任务结果
     */
    async getVideoTaskResult(taskId, reqKey = "jimeng_vgfm_t2v_l20") {
        let lastError = null;
        const retries = 1; // 只重试一次
        let retryCount = 0;
        while (retryCount <= retries) {
            ...
            try {
                ...
                if (response.status === 200) {
                    if (response.data.status === 10000 || response.data.code === 10000) {
                        // 从响应中提取任务ID
                        const taskId = (_a = response.data.data) === null || _a === void 0 ? void 0 : _a.task_id;
                        if (!taskId) {
...
exports.JimengClient = JimengClient;
